...
function readMember(node, curStatus, c) {
    ...
    constraints.push({OBJ: objAVal,
                      PROP: name,
                      READ_TO: ret});
    objAVal.propagate(new cstr.ReadProp(name, ret));
    ...
    var constraintGenerator = walk.make({
        ...
        AssignmentExpression: function (node, curStatus, c) {
            ...
            if (node.operator === '=') {
                ...
                objAVal.propagate(new cstr.WriteProp(pName, rhsAVal));
                return rhsAVal;
            }
            ...
            recvAndRet[0].propagate(new cstr.WriteProp(pName, resAVal));
            return resAVal;
        },
        ...
        ArrayExpression: function (node, curStatus, c) {
            ...
            for (var i = 0; i < node.elements.length; i++) {
                ...
                constraints.push({OBJ: ret, PROP: '<i>', AVAL: eltAVal});
                ret.propagate(new cstr.WriteProp(i, eltAVal));
                ret.propagate(new cstr.WriteProp('<i>', eltAVal));
            }
            return ret;
        },
