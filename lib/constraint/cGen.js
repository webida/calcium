    ...
    const constraintGenerator = walk.make({
        ...
        AssignmentExpression: function (node, curStatus, c) {
            ...
            if (node.left.type !== 'MemberExpression') {
                ...
                const propAcc = propAccess(node.left);
                if (node.operator === '=') {
                ...
                const resAVal = new types.AVal;
                const recvAndRet = readMember(node.left, curStatus, c);
                if (node.operator === '+=') {
                    // concatenating update
                    constraints.push({
        ...
        FunctionDeclaration: function (node, curStatus, c) {
            ...
            const sc0 = curStatus.sc.removeInitialCatchBlocks();
            if (!node.fnInstances) {
                node.fnInstances = [];
            }
            let fnInstance = null;
        ...
        TryStatement: function (node, curStatus, c) {
            ...
            // for try block
            const tryStatus = changedStatus(curStatus, 'exc', excAVal);
            c(node.block, tryStatus, undefined);

            // for catch block
            const catchStatus = changedStatus(curStatus, 'sc', catchBlockSC);
        ...
        CallExpression: function (node, curStatus, c) {
            ...
            if (node.callee.type === 'MemberExpression') {
                ...
                //   DELTA: newDelta
                // });
                const recvAndRet = readMember(node.callee, curStatus, c);
        ...
        MemberExpression: function (node, curStatus, c) {
            const recvAndRet = readMember(node, curStatus, c);
            return recvAndRet[1];
        },
