...
function addConstraints(astNode, initStatus, newRtCX) {
        ...
        ArrayPattern: (node, curStatus, source) => {
            for (let i = 0; i < node.elements.length; i++) {
                ...
                if (elt) {
                    ...
                    const indexAVal = Ĉ.get(elt, curStatus.delta);
                    source.propagate(new cstr.ReadProp(String(i), indexAVal));
                    connectPattern(elt, curStatus, indexAVal);
                }
            }
        },
    ...
    function connectPattern(node, curStatus, source) {
